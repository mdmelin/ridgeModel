function [out,out2] = ridgeModel_returnDesignMat(cPath,Animal,Rec,glmFile,dType)

% Return design matrix (without state) and the corresponding state
% probabilties. 
...
out = [];
nTrials = size(fullR,1)/75;
for i = 1:nTrials
    inds = i*75-74:i*75;
    onetrial = fullR(inds,:);
    onetrial = reshape(onetrial,[],1);
    out = [out,onetrial];
end
alltrials = [];
for i = 1:length(trialIdx)/75
    inds = i*75-74:i*75;
    onetrial = trialIdx(inds);
    alltrials = [alltrials, onetrial];
end
take = mean(alltrials,1) == 0;
out2 = postprobs(take,:);
end
