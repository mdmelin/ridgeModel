function data = rateDisc_vidResamp(data, trialTime, targRate)
...
if sum(~isnan(data(:))) > 1 %needs at least 2 valid datapoints
    ...
    trialTime = trialTime - min(trialTime);
    data = NaN(ceil(trialTime(end)*targRate + 1), size(data,2));
end