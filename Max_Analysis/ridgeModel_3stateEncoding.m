function ridgeModel_3stateEncoding(cPath,Animal,Rec,glmFile,dType)
...
%choiceIdx = rateDisc_equalizeTrials(choiceIdx, (inds == 1)', (inds == 3)',inf, true);
...
save([cPath glmFile '3withstate.mat'],'regIdx','rejIdx','Vm', 'fullBeta', 'fullIdx', 'fullR', 'fullLabels', 'fullRidge', 'regLabels', 'fullMap', 'fullMovie','-v7.3'); %this saves model info based on all regressors

%% Run WITHOUT state - should mirror original model

labels = {'attentive','lbias','rbias'};
...
save([cPath glmFile '3nostate.mat'],'regIdx','rejIdx','Vm', 'fullBeta', 'fullIdx', 'nostateR', 'fullLabels', 'fullRidge', 'regLabels', 'fullMap', 'fullMovie','-v7.3'); %this saves model info based on original model

%% Run with engaged ONLY

labels = {'attentive'};
labels = regLabels(sort(find(ismember(regLabels,labels)))); %make sure  in the right order

[Vm, fullBeta, stateR, fullIdx, fullRidge, fullLabels, fullMap, fullMovie] = crossValModel(labels);
save([cPath glmFile '3attentive.mat'],'regIdx','rejIdx','Vm', 'fullBeta', 'fullIdx', 'stateR', 'fullLabels', 'fullRidge', 'regLabels', 'fullMap', 'fullMovie','-v7.3'); %this saves model info based on state only
...
try
    [Vm, fullBeta, stateR, fullIdx, fullRidge, fullLabels, fullMap, fullMovie] = crossValModel(labels);
    save([cPath glmFile '3lbias.mat'],'regIdx','rejIdx','Vm', 'fullBeta', 'fullIdx', 'stateR', 'fullLabels', 'fullRidge', 'regLabels', 'fullMap', 'fullMovie','-v7.3'); %this saves model info based on state only
catch
    fprintf('NO TRIALS!');
end
...
try
    [Vm, fullBeta, stateR, fullIdx, fullRidge, fullLabels, fullMap, fullMovie] = crossValModel(labels);
    save([cPath glmFile '3rbias.mat'],'regIdx','rejIdx','Vm', 'fullBeta', 'fullIdx', 'stateR', 'fullLabels', 'fullRidge', 'regLabels', 'fullMap', 'fullMovie','-v7.3'); %this saves model info based on state only
catch
    fprintf('NO TRIALS!');
end
